...
bptest(capm_b) # o modelo CAPM rejeita a hipótese nula do teste de Breusch-Pagan, o que o caracteriza como heterocedástico

bptest(ffrench_b) # o modelo de Fama-French rejeita a hipótese nula, o que o caracteriza como heterocedástico

##### ------ Testes de Correlação Serial ------ #####

# Teste de Durbin-Watson

dwtest(capm_b) # O teste de Durbin-Watson em ambos os modelos indica que não há correlação serial, por aceitarem a hipótese nula.

dwtest(ffrench_b)

# Teste de Breusch-Godfrey

bgtest(capm_b) # O teste de Breusch-Godfrey indica que há correlação serial no modelo CAPM, rejeitando a hipótese nula. 

bgtest(ffrench_b) # Já o teste de Breusch-Godfrey indica que não haveria correlação serial no modelo Fama-French, aceitando a hipótese nula.
...
bptest(capm_b_corrigido) # o modelo CAPM rejeita a hipótese nula do teste de Breusch-Pagan, o que o caracteriza como heterocedástico

bptest(ffrench_b_corrigido) # o modelo de Fama-French rejeita a hipótese nula, o que o caracteriza como heterocedástico

##### ------ Testes de Correlação Serial ------ #####

# Teste de Durbin-Watson

dwtest(capm_b_corrigido) # O teste de Durbin-Watson em ambos os modelos indica que não há correlação serial, por aceitarem a hipótese nula.

dwtest(ffrench_b_corrigido)

# Teste de Breusch-Godfrey

bgtest(capm_b_corrigido) # O teste de Breusch-Godfrey em ambos os modelos indica que agora não há correlação serial, por aceitarem a hipótese nula. 

bgtest(ffrench_b_corrigido) 

# A retirada dos outliers foi responsável pela correção da correlação serial em ambos os modelos, a partir dos de seus dois testes mais importantes.
# Entretanto, a heterocedasticidade (variância inconstante dos resíduos) ainda não foi corrigida.

# Já no modelo Fama-French, é impossível retirar a heterocedasticidade pois não há erros na especificação das formas funcionais. Concluimos que ela
# deve estar atrelada à natureza da série temporal analisada.

##### ------ QUESTÃO 3 ------ #####

# O que caracteriza o CAPM como heterocedástico é a má especificação das formas funcionais. Por isso, fizemos testes diversos com a variável MKT elevada
# a outras potências e identificamos que MKT^4 é muito significante. Além disso, o modelo novo corrige o problema. 
...
# antes
jarqueberaTest(residuals(capm_b))
jarqueberaTest(residuals(ffrench_b))
